...
#| label: wd-1-xlsx
# CheckIn 2.3
ages_xlsx <- read_xlsx(here::here("week_2", "check-ins", "Ages_Data", "ages.xlsx"), sheet = "ages")
```
...
#| label: wd-6-semi
# Lab 5
# this question is pretty heavily modified. I originally just used left joins to join the drivers_license and facebook_event_checkin then filtered the entire joined table based on the clues. Now, I am pre-filtering each data set to select the relevant info from each table, then semi-joining them with person to only keep the relevant rows. I think the original version of this was slightly cleaner and doesn't save extra objects, but this works fine.
# People whose license matches the description
lic_candidates <- drivers_license |>
  filter(
    height >= 65, height <= 67,
    hair_color == "red",
    car_make == "Tesla",
...
# People who checked into the SQL Symphony Concert 3 times in Dec 2017
concert_3x <- facebook_event_checkin |>
  mutate(date = ymd(date)) |>
  filter(
  ...
  count(person_id, name = "cnt") |>
  filter(cnt == 3)

person |>
  semi_join(lic_candidates, by = join_by(license_id == id)) |>
  semi_join(concert_3x, by = join_by(id == person_id)) 
...
#| label: wd-6-anti
# Lab 5
# Modified to use an antijoin rather than a left join + filter. Originally I was filtering to see if Miranda had any interviews, now I'm doing an antijoin to return if the people named Miranda who do not have any interviews.
person |>
...
#| label: dvs-6-ex-2
# Lab 9: Question 6
# originally I used percent() which is superceded, so I switched it to label_percent()
evals_kbl |>
  rename(
  ...
  mutate(
    Percent = label_percent(Percent, accuracy = 0.01)
  ) |>
